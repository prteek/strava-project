...
fitness_data_files = [
    "./data/20230101-20230331_fitness.csv",
    "./data/20230401-20230507_fitness.csv",
]

df_fitness = (
    pd.concat([read_and_parse_fitness_data(i) for i in fitness_data_files])
    .sort_values("date")
...
df_merged = pd.merge(df_fitness, df_activities, on="date", how="outer").astype(
    {
        "fitness_score": float,
        "date": "datetime64[ns]",
        "id": float,
...
plt.plot(
    df_merged["date"].astype("datetime64"),
    df_merged["fitness_score"],
    label="Fitness date",
)
plt.scatter(
    df_merged["start_timestamp"].dt.date.astype("datetime64"),
    df_merged["fitness_score"],
    c="r",
    label="Act date",
)
...
df_rest = (
    df_merged.get(["fitness_score", "id", "date"])
    .assign(
        days_diff=lambda x: (x["date"].shift(-1) - x["date"]).dt.days,
...
plt.scatter(
    df_rest["days_diff"], df_rest["fitness_score_final"] - df_rest["fitness_score"]
)
plt.xlim(0, 10)
plt.ylim(-4, 1)
...
plt.plot(
    x[:, 0], decay_model(x[:, 0], x[:, 1], popt), "s", label="Fitted model", alpha=0.8
)
...
df_plot = (
    ...
    .get(
        ["suffer_score", "score_diff", "fitness_score_initial", "fitness_score", "date"]
    )
)
